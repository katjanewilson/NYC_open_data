...
school_nearest <- matchit(formula = treatment_indicator ~ Economic.Need.Index +
                            + X..Poverty + X..White #+ Total.Enrollment
...
working_data_nomiss<- working_data %>%
  select(Percent_Attendance, treatment_indicator, X..Poverty, 
         Economic.Need.Index, X..White, X..Students.with.Disabilities, ps, iptw, stable.iptw,
         treatment_indicator)
...
boxplot(working_data_nomiss$X..White ~ working_data_nomiss$treatment_indicator)
...
mod2 <- matchit(formula = treatment_indicator ~ 
                 X..Poverty + X..White  #Total.Enrollment
                , data = working_data_nomiss,
