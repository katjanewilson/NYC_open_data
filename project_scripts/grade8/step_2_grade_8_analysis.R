...
third_grade_data <- read.csv("../data/third_grade_data_2017_Grade8.csv")
...
#                                                 1, 0))
third_grade_data <- third_grade_data %>%
  mutate(only_gen_ed = ifelse(
                                self_contained_option == 0 &
                                ICT_option == 0 &
                                GenEd == 1,
                              1, 0))
write.csv(third_grade_data, '/cloud/project/data/third_grade_data_cleaned_Grade8.csv')
######
# STEP 3
...
working_data %>%
  group_by(gifted_talented_option) %>%
  summarise(mean_attendance = mean(Percent_Attendance))
write.csv(working_data, file = '/cloud/project/data/third_grade_data_cleaned_Grade8.csv')
